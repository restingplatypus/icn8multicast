...
LteRadioBearerTag::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteRadioBearerTag")
	...
	.AddAttribute ("guid", "The GUID of the content requested by UE",
				   UintegerValue (0),
				   MakeUintegerAccessor (&LteRadioBearerTag::GetGuid),
				   MakeUintegerChecker<uint16_t> ())
  ;
  return tid;
}
...
LteRadioBearerTag::LteRadioBearerTag (uint16_t rnti, uint8_t lcid, uint16_t guid)
  : m_rnti (rnti),
    m_lcid (lcid),
	m_guid (guid)
{
...
LteRadioBearerTag::LteRadioBearerTag (uint16_t rnti, uint8_t lcid, uint8_t layer, uint16_t guid)
: m_rnti (rnti),
  m_lcid (lcid),
  m_layer (layer),
  m_guid (guid)
...
void
LteRadioBearerTag::SetGuid(uint16_t guid)
{
	m_guid = guid;
}
...
uint16_t
LteRadioBearerTag::GetGuid () const
{
  return m_guid;
}
...
//  os << "rnti=" << m_rnti << ", lcid=" << (uint16_t) m_lcid << ", layer=" << (uint16_t)m_layer;
	os << "rnti=" << m_rnti << ", lcid=" << (uint16_t) m_lcid << ", layer=" << (uint16_t)m_layer << ", guid=" << (uint16_t) m_guid;
}
