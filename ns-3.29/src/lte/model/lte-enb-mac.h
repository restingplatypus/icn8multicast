...
public:
  ...
  */
  ...
  typedef void (* AxRntiTracedCallback)(const uint16_t guid, uint16_t rnti);
...
private:
  ...
  */
  ...
  /**
   * Created new trace source that fires up anytime a new rnti is added to a corresponding guid
   */
  TracedCallback<uint16_t, uint16_t> m_axTrace;
  ...
  std::map<uint16_t, uint16_t> m_guidRntiMap;

  std::map<uint16_t, Time> m_guidTimerMap;

  std::map<uint16_t, std::vector<uint16_t>> m_axRntiMap;
