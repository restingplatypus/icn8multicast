...
//    	{
//    		NS_LOG_INFO("Downlink transmission for IMSI 2 and 3 stopped.");
//    	}
//
//    	else
//    	{
...
//    	params.pdcpSdu->PeekHeader(pdcpHeader);
//    	uint16_t guid = pdcpHeader.GetData();
//    	NS_LOG_INFO("GUID Tx at SendData " << guid );


    	/**
    	 * In case the above experiment doesn't work, just uncomment the entire commented block from {} and comment out your modifications
    	 */
    {
    	NS_LOG_LOGIC ("queueing data on PDCP for transmission over the air");
    	LtePdcpSapProvider::TransmitPdcpSduParameters params;
...
//	  params.pdcpSdu->PeekHeader(pdcpHeader);
//	  uint16_t guid = pdcpHeader.GetData();
//	  NS_LOG_INFO("GUID Rx at UE manager " << guid);
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("RsrqFilterCoefficient",
                   ...
                   MakeUintegerChecker<uint8_t> (0))
	.AddAttribute ("TxWaitDelay",
				   "The transmission delay due to aggregation",
				   //TimeValue(Seconds(3.0)),
				   DoubleValue(3.0),
				   MakeDoubleAccessor(&LteEnbRrc::m_txWaitDelay),
				   MakeDoubleChecker<double> ())
				   //MakeTimeAccessor (&LteEnbRrc::m_txWaitDelay),
				   //MakeTimeChecker())
