...
LteUeMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeMac")
	...
	.AddAttribute("Alpha",
				  "The Zipf Parameter",
				  DoubleValue(1.5),
				  MakeDoubleAccessor(&LteUeMac::m_alpha),
				  MakeDoubleChecker<double>(0, 3))
	;
  return tid;
}
...
LteUeMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  x->SetAttribute("Alpha", DoubleValue(m_alpha));
  uint16_t guid;
  guid = static_cast<uint16_t>(x->GetValue ());

  NS_LOG_INFO("GUID Tx at UE with RNTI " << params.rnti << " is " << guid);

  LteRadioBearerTag tag (params.rnti, params.lcid, 0 , guid);
