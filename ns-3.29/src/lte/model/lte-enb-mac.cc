...
LteEnbMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteEnbMac")
	...
	.AddAttribute ("WaitPeriod",
				   "Wait period for aggregating the multicast requests",
				   TimeValue(Seconds(3.0)),
				   MakeTimeAccessor (&LteEnbMac::m_waitPeriod),
				   MakeTimeChecker())
	...
	.AddTraceSource ("AxRnti",
					 "RNTI values accepted for Group Membership for a particular GUID",
					 MakeTraceSourceAccessor(&LteEnbMac::m_axTrace),
					 "ns3::LteEnbMac::AxRntiTracedCallback")
...
//  m_frameNo = frameNo + 500;
//  m_subframeNo = subframeNo + 500;
...
//     }
  ...
  uint16_t guid_rx = tag.GetGuid();
  NS_LOG_INFO("eNB MAC received GUID " << (uint16_t)guid_rx << " from RNTI " << (uint16_t) tag.GetRnti());


  std::map<uint16_t, uint16_t> :: iterator guidIt = m_guidRntiMap.find(guid_rx);

  if(guidIt == m_guidRntiMap.end()) // this means that you've encountered this guid_rx for the first time. So logically you should start your timer now.
  {
	  m_guidTimerMap[guid_rx] = Simulator::Now();
	  NS_LOG_INFO("GUID " << guid_rx << " encountered for first time at " << m_guidTimerMap[guid_rx] << ". Adding GUID " << guid_rx << " and RNTI " << tag.GetRnti());
	  uint16_t rnti = tag.GetRnti ();
	  m_guidRntiMap[guid_rx] = rnti;
	  m_axRntiMap[guid_rx].push_back(rnti);
	  ...
	  std::vector<uint16_t> rntiVec = m_axRntiMap[guid_rx];

	  /**
	   * fire up the trace
	   */
	  m_axTrace(guid_rx, rnti);
  }
  else
  {
	  if(tag.GetRnti() == guidIt->second)
	  {
		  NS_LOG_INFO("GUID " << guid_rx << " found and Sending for RNTI " << tag.GetRnti());
		  uint16_t rnti = tag.GetRnti ();
		  uint8_t lcid = tag.GetLcid ();
		  std::map <uint16_t, std::map<uint8_t, LteMacSapUser*> >::iterator rntiIt = m_rlcAttached.find (rnti);
		  NS_ASSERT_MSG (rntiIt != m_rlcAttached.end (), "could not find RNTI" << rnti);
	  ...
	  {
		  Time difference = Simulator::Now() - m_guidTimerMap[guid_rx];
		  NS_LOG_INFO("Time Difference = " << difference);

		  if( ( Simulator::Now() - m_guidTimerMap[guid_rx] ) < m_waitPeriod )
		  {
			  NS_LOG_INFO("RNTI " << tag.GetRnti() <<" accepted in group for GUID " << guid_rx);
			  m_axRntiMap[guid_rx].push_back(tag.GetRnti());
			   ...
			   *  Fire up the trace
			   */
			  m_axTrace(guid_rx, tag.GetRnti());
		  }
		  else
		  {
			  NS_LOG_INFO("Dropping RNTI " << tag.GetRnti() << " for group membership corresponding to GUID " << guid_rx);
		  }
...
LteEnbMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  LteRadioBearerTag tag (params.rnti, params.lcid, params.layer, 0);
...
//  Time delay = Simulator::Now() + NanoSeconds(5000000000);
//  Simulator::Schedule(delay, &LteEnbPhySapProvider::SendMacPdu, m_enbPhySapProvider, params.pdu);
...
LteEnbMac::DoReportBufferStatus (LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  req.imsi = params.imsi;
  Time delay = Simulator::Now() + Seconds(5.0);
  //Simulator::Schedule(Seconds(delay), &FfMacSchedSapProvider::SchedDlRlcBufferReq, m_schedSapProvider, req);
