...
void
LtePdcp::DoTransmitPdcpSdu (Ptr<Packet> p, uint64_t imsi)
{
 ...
 // p->AddHeader (pdcpHeader);
  ...
  uint16_t value;
  value = static_cast<uint16_t>(x->GetValue ());
 // uint16_t val = value;
  //IcnHeader icnHeader;
  //icnHeader.SetData(value);
  pdcpHeader.SetData(value);
  NS_LOG_INFO("GUID Tx at PDCP = " << pdcpHeader.GetData());
...
//  LtePdcpHeader pdcpH;
//  p1->RemoveHeader(pdcpH);
//  uint16_t guid = pdcpH.GetData();
//
//  NS_LOG_INFO("GUID at PDCP = " << guid);
...
//
//  NS_LOG_INFO("GUID  received at PDCP = " << guid);
  NS_LOG_LOGIC ("PDCP header: " << pdcpHeader);
