...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("ICN8_3");
...
int main (int argc, char *argv[])
{
	...
	std::string tracefilePrefix = "icn8_3";
	...
	/****************************************************************************************************************
	 **************************Installing packet sink and bulk application at server**********************************
	 ****************************************************************************************************************/
	uint16_t dlPort = 1100;
	uint16_t ulPort = 2000;
	ApplicationContainer clientAppsUe;
	ApplicationContainer clientRem;
	ApplicationContainer sinkApps;

	for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
	{
		//--------------------Installing sink application at remote host----------------------------------------------------------
		++ulPort;
		PacketSinkHelper ulPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), ulPort));
		sinkApps.Add (ulPacketSinkHelper.Install (remoteHost));


		//----------------------Installing bulk send app at remote host------------------------------------------------------------
		BulkSendHelper dlClientRem ("ns3::TcpSocketFactory", InetSocketAddress (ueIpIface.GetAddress (u), dlPort));
		dlClientRem.SetAttribute("MaxBytes", UintegerValue(contentSize));
		clientRem.Add(dlClientRem.Install(remoteHost));


		//-------------------Installing sink application at UES-----------------------------------------------------------------------
		PacketSinkHelper dlPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), dlPort));
		sinkApps.Add (dlPacketSinkHelper.Install (ueNodes.Get (u)));


		// ------------------Installing client applications at UEs---------------------------------------------------------------------------
		...
		ulClient.SetAttribute("PacketSize", UintegerValue(64));
		clientAppsUe.Add (ulClient.Install (ueNodes.Get(u)));
		clientAppsUe.Start(Seconds(startTime));
		clientAppsUe.Stop(Seconds(totalTime));
	}

	sinkApps.Start(Seconds(0.0));
	sinkApps.Stop(Seconds(totalTime));
	clientRem.Start(Seconds(0.0));
	clientRem.Stop(Seconds(totalTime));
