...
int main (int argc, char *argv[])
{
	...
	//trace files prefix
	std::string tracefilePrefix = "icn2";
	...
	for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
	{
		...
		OnOffHelper dlOnOffHelper("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), dlPort));
		...
		dlOnOffHelper.SetAttribute("OnTime", StringValue(pauseConstantRandomVariableStream.str()));
		dlOnOffHelper.SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=0.1]"));
		dlOnOffHelper.SetAttribute ("DataRate", DataRateValue (DataRate ("0.4Mbps")));
		dlOnOffHelper.SetAttribute ("PacketSize", UintegerValue (1500));
