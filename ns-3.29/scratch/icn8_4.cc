...
#include "../src/applications/model/tcp-server-application.h"
#include "../src/applications/model/tcp-client-application.h"
...
int main (int argc, char *argv[])
{
	...
	Time waitPeriod = Seconds(1.0);
	double meanArrival = 0.8;
//	Time txWaitDelay = Seconds(3.0);
	double txWaitDelay = 1.0;
	uint16_t dlBandwidth = 50;
	uint16_t ulBandwidth = 50;
	//double enbTxPowerDbm = 46;
	double alpha = 1.5;
	uint16_t f_density = 3; // in veh/mile/lane
	uint32_t contentSize=150*1024; // content size in bytes
	...
	p2ph.SetQueue ("ns3::DropTailQueue<Packet>", "MaxSize", QueueSizeValue (QueueSize ("100p"))); // p in 100p stands for packets
...
//	}
	 ...
	 *
	 */
	TcpServerApplicationHelper echoServer (InetSocketAddress (Ipv4Address::GetAny (), 9));
	echoServer.SetAttribute ("MaxTxBytes", UintegerValue (contentSize));
	...
	TcpClientApplicationHelper echoClient(InetSocketAddress(internetIpIfaces.GetAddress(1), 9));
//	echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
//	echoClient.SetAttribute ("PacketSize", UintegerValue (64));
	echoClient.SetAttribute ("MaxRxBytes", UintegerValue (contentSize));
...
//		icnServer->TraceConnectWithoutContext("Rx", MakeBoundCallback (&ClientRx, stream2));
