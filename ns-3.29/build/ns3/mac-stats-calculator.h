...
public:
  ...
  void SetWindowTraceFilename (std::string windowFileName);

  std::string GetWindowTraceFilename (void);
  ...
  /**
   * Trace sink for the ns3::LteEnbMac::AxRnti trace source
   */
  //static void WindowTraceCallback (Ptr<MacStatsCalculator> macStats, std::string path, uint16_t guid, uint16_t rnti);
//  static void WindowTraceCallback (uint16_t guid, uint16_t rnti);
